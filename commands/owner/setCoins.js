...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("setcoins")
    .setDescription("Imposta i coin di un utente (Bot owner only)")
    .addStringOption((option) =>
      option.setName("utente").setDescription("ID dell'utente target").setRequired(true)
    )
    .addIntegerOption((option) =>
      option.setName("quantità").setDescription("Numero di coin da impostare").setRequired(true)
    ),
  ...
  async execute(interaction) {
    ...
    try {
      ...
      return interaction.reply({
        content: `Impostati i coin di ${userCoins.username} a ${amount}.`,
        ephemeral: true,
      });
    } catch (error) {
      console.error("Errore durante l'impostazione dei coin:", error);
      return interaction.reply({
