...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("addcoins")
    .setDescription("Aggiungi coin a un utente (Bot owner only)")
    .addStringOption((option) =>
      option.setName("utente").setDescription("ID dell'utente target").setRequired(true)
    )
    .addIntegerOption((option) =>
      option.setName("quantità").setDescription("Numero di coin da aggiungere").setRequired(true)
    ),
  ...
  async execute(interaction) {
    ...
    try {
      ...
      return interaction.reply({
        content: `Aggiunti ${amount} coin a ${userCoins.username}. Totale: ${userCoins.amount}`,
        ephemeral: true,
      });
    } catch (error) {
      console.error("Errore durante l'aggiunta dei coin:", error);
      return interaction.reply({
